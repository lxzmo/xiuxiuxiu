...
extension AuthUIBuilder {
    ...
    func buildFullScreenModel(config: AuthUIConfig) -> TXCustomModel {
        ...
        if #available(iOS 13.0, *) {
            model.navColor = config.navColor?.uicolor() ?? UIColor.systemBackground
        }else{
        ...
        sloganTextAtrributes.updateValue((config.sloganTextColor ?? "#999999").uicolor(), forKey: NSMutableAttributedString.Key.foregroundColor)
        ...
        model.loginBtnFrameBlock = { screenSize, _, _ -> CGRect in
            ...
            loginButtonOffsetY = config.loginBtnFrameOffsetY ?? Float(screenSize.height) * 0.4 + kPadding
        ...
        model.privacyConectTexts = [config.privacyConnectTexts ?? "&", config.privacyConnectTexts ?? "&"]
        ...
        model.privacyFrameBlock = { screenSize, _, _ -> CGRect in
            ...
            if kHorizontal! {
                ...
                offsetY = CGFloat(config.privacyFrameOffsetY ?? screenHeight - kPadding - kBottomInset - 80)
            } else {
                offsetY = CGFloat(config.privacyFrameOffsetY ?? screenHeight - kPadding - 80 - 80)
            }

            let offsetX = CGFloat(screenSize.width * 0.07)

            return CGRect(x: offsetX, y: offsetY, width: screenSize.width * 0.86, height: 48)
        }
